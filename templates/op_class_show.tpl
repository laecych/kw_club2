<h2 class="text-center">
    <span class="badge badge-info bg-info"><{$cate_id_title}></span>
    <{$class_title}>
    <!--是否開班-->
...
<div class="alert alert-info text-center">
    <{if $is_full}>
        <a href="#" class="btn btn-danger disabled"><i class="fa fa-user-plus" aria-hidden="true"></i>
            <{$smarty.const._MD_KWCLUB_FULL_REGISTRATION}></a>
    <{elseif $class_regnum >= $class_member}>
        <a href="index.php?op=reg_form&class_id=<{$class_id}>&is_full=1" class="btn btn-warning"><i class="fa fa-solid fa-ban"></i>
            <{$smarty.const._MD_KWCLUB_SIGNUP_TO_MAKE_UP}></a>
    <{elseif $chk_time}>
        <a href="index.php?op=reg_form&class_id=<{$class_id}>" class="btn btn-primary"><i class="fa fa-user-plus" aria-hidden="true"></i>
            <{$smarty.const._MD_KWCLUB_SIGNUP}></a>
...
<{if $smarty.session.isclubAdmin || $smarty.session.isclubUser }>
    <div class="alert alert-success text-center">
        <a href="club.php?club_year=<{$club_year}>" class="btn btn-primary" <{if !$can_operate}>data-toggle="tooltip"  data-placement="bottom" data-bs-toggle="tooltip"  data-bs-placement="bottom" title="<{$smarty.const._MD_KWCLUB_OVER_END_TIME}>" disabled<{/if}>>
        ...
        <{if $smarty.session.isclubAdmin || $uid == $class_uid }>
            ...
            <a href="club.php?class_id=<{$class_id}>" class="btn btn-warning" <{if !$can_operate}>data-toggle="tooltip"  data-placement="bottom" data-bs-toggle="tooltip"  data-bs-placement="bottom" title="<{$smarty.const._MD_KWCLUB_OVER_END_TIME}>" disabled<{/if}>>
