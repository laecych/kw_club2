<form action="config.php" method="post" id="myForm" enctype="multipart/form-data"  role="form">
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <{if $isRegisted}>
            <{if $club_isfree==1}><{$smarty.const._MD_KWCLUB_FREE_APPLY}><{else}><{$smarty.const._MD_KWCLUB_LOGIN_APPLY}><{/if}>
            <input type="hidden" name='club_isfree' value='<{$club_isfree}>'>
            <{else}>
            <label class="radio-inline">
                <input type='radio' name='club_isfree' id='club_isfree1' title='<{$smarty.const._MD_KWCLUB_FREE_APPLY}>' value='1' <{if $club_isfree==1}>checked<{/if}> >
                <{$smarty.const._MD_KWCLUB_FREE_APPLY}><{$smarty.const._MD_KWCLUB_FREE_APPLY_DESC}>
            </label>
            <label class="radio-inline">
                <input type='radio' name='club_isfree' id='club_isfree2' title='<{$smarty.const._MD_KWCLUB_LOGIN_APPLY}>' value='0' <{if $club_isfree==0}>checked<{/if}>>
    ...
    <!-- 顯示方式：是否顯示正備取 -->
    <div class="form-group row">
    <label for="club_isshow" class="col-sm-2 col-form-label text-sm-right"><{$smarty.const._MD_KWCLUB_ISSHOW}></label>
        <div class="col-sm-10">
            <label class="radio-inline">
                <input type='radio' name='club_isshow' id='club_isshow1' title='<{$smarty.const._MD_KWCLUB_ISSHOW_YES}>' value='0' <{if $club_isshow==0}>checked<{/if}>>
                <{$smarty.const._MD_KWCLUB_ISSHOW_YES}>
            </label>
            <label class="radio-inline">
                <input type='radio' name='club_isshow' id='club_isshow2' title='<{$smarty.const._MD_KWCLUB_ISSHOW_NO}>' value='1' <{if $club_isshow==1}>checked<{/if}>>
                <{$smarty.const._MD_KWCLUB_ISSHOW_NO}>(<{$smarty.const._MD_KWCLUB_ISSHOW_DESC}>)
            </label>
        </div>
    </div>
    <!--候補人數-->
    ...
    <!--排序-->
    <div class="form-group row">
        <label for="club_sort" class="col-sm-2 col-form-label text-sm-right">
            <{$smarty.const._MD_KWCLUB_SORT}>
        </label>
        <div class="col-sm-4">
            <input class='form-control validate[required]' type='text' name="club_sort" id="club_sort" title="<{$smarty.const._MD_KWCLUB_SORT}>" value='<{$club_sort}>' placeholder="<{$smarty.const._MD_KWCLUB_KEYIN}><{$smarty.const._MD_KWCLUB_SORT}>">
        </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-6">
                ...
                <{$token_form}>
                    ...
                    <input type="hidden" name="op" value="<{$next_op}>">
                    <input type="hidden" name="club_id" value="<{$club_id}>">
                    <input type="hidden" name='isRegisted' value='<{$isRegisted}>'>
