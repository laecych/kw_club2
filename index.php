...
use XoopsModules\Tadtools\XoopsFormFile;
...
function reg_form($class_id)
{
    ...
    if('0' == $_SESSION['club_isfree'] && !$_SESSION['isclubAdmin']){ //login openID
        if (!$xoopsUser){
            redirect_header($_SERVER['PHP_SELF'], 3, _MD_KWCLUB_NEED_OPENID);
        }
        $school_code = $xoopsModuleConfig['school_code'];
        if(empty($school_code))  redirect_header($_SERVER['PHP_SELF'], 3, _MD_KWCLUB_NEED_CODE);
        
        //取得報名資料
        $uid = $xoopsUser->uid();
        $sql = 'select `uname`,`name`, `user_from`,`user_sig` from `' . $xoopsDB->prefix('users') . "`  where `uid` = '{$uid}' and `user_icq`='student' and `user_intrest`='{$school_code}' ";
        $result = $xoopsDB->query($sql);
        list($uname,$name,$grade,$classnum) = $xoopsDB->fetchRow($result);
        if( $uname && $name && $grade && $classnum){
            $xoopsTpl->assign('user', $uname);
            $xoopsTpl->assign('name', $name);
            $xoopsTpl->assign('grade', $grade);
            $xoopsTpl->assign('classnum', $classnum);
        }else{
            redirect_header($_SERVER['PHP_SELF'], 3, _MD_KWCLUB_NOT_SCHOOL);
        }
    }

    $class_arr = get_club_class($class_id);
    $xoopsTpl->assign('class', $class_arr);
    $class_grade_arr = explode('、', $class_arr['class_grade']);
    ...
    //比對年級是否正確
    if('0' == $_SESSION['club_isfree'] ){
        
        if(! in_array($xoopsModuleConfig['school_grade'][$grade],  $class_grade_arr ) ){
            redirect_header($_SERVER['PHP_SELF'], 3, _MD_KWCLUB_GRADE_ERROR);
        }
    }
    ...
    $xoopsTpl->assign('school_grade', $xoopsModuleConfig['school_grade']);
    //幫學生報名
    $_SESSION['isclubAdmin'] ? $reg_isfree = 1 : $reg_isfree=$_SESSION['club_isfree'];
    $xoopsTpl->assign('reg_isfree',$reg_isfree);
...
function insert_reg()
{
    ...
    if (!$_SESSION['isclubAdmin']) {
        kw_club_chk_time('', false, $club_info['club_start_date'], $club_info['club_end_date']);
    ...
    $reg_number = $myts->addSlashes($_POST['reg_number']);
    ...
    $sql = " INSERT INTO `" . $xoopsDB->prefix('kw_club_reg') . "` (
        `class_id`,`reg_uid`, `reg_name`, `reg_grade`, `reg_class`, `reg_number`,`reg_parent`, `reg_tel`, `reg_isreg`, `reg_datetime`,  `reg_ip`) VALUES
        (
            '{$class_id}',
            '{$reg_uid}',
            '{$reg_name}',
...
{
    global $xoopsDB, $xoopsTpl,$today,$xoopsUser,$xoopsModuleConfig;
    //查詢年度
    $club_year = (empty($club_year) && isset($_SESSION['club_year'])) ? $_SESSION['club_year'] : $club_year;
    $club_info = get_club_info($club_year);

    if($club_info['club_isfree']==0 && !$_SESSION['isclubUser']  )//登入報名
    {  
        $uid = $xoopsUser->uid();
        $sql = 'select `uname`, `user_intrest` from `' . $xoopsDB->prefix('users') . "`  where `uid` = '{$uid}' and `user_icq`='student' ";
        $result = $xoopsDB->query($sql);
        list($uname, $school_code) = $xoopsDB->fetchRow($result);
        if( $school_code != $xoopsModuleConfig['school_code'])
             redirect_header("index.php?club_year={$club_year}", 3, _MD_KWCLUB_NOT_SCHOOL);
        $reg_uid = empty($reg_uid) ? $uname : $reg_uid;
    }
    ...
    if (empty($reg_uid)) {
        ...
        $sql = 'select a.*, b.*, c.`club_end_date`,c.`club_isfree`,c.`club_isshow` from `' . $xoopsDB->prefix('kw_club_reg') . '` as a
        join `' . $xoopsDB->prefix('kw_club_class') . '` as b on a.`class_id` = b.`class_id`
...
function class_show($class_id = '')
{
    ...
    $xoopsTpl->assign('isshow', $_SESSION['club_isshow']);

    //檢查報名是否可行
    $xoopsTpl->assign('chk_time',kw_club_chk_time('return'));
    //超過報名截止時間即停止報名及修改
    // $xoopsTpl->assign('can_operate',kw_club_chk_time('return', true));
    $xoopsTpl->assign('can_operate', true);
    ...
    if ($class_regnum > 0) {
        //取得報名資料
        $all_reg = get_club_class_reg($club_year, $class_id); //default order by reg_datetime
...
function teacher_list($club_year = '')
{
    ...
    while (false !== ($data = $xoopsDB->fetchArray($result))) {
        ...
        //teacher upload data
        $sql_file = 'select * from `' . $xoopsDB->prefix('kw_club_files_center') . "` where `col_sn`='{$id}' order by `sort`";
        $result_file = $xoopsDB->query($sql_file) or web_error($sql_file);
        $teachers_arr[$id]['files']=[];
        $i=0;
        while (false !== ($file = $xoopsDB->fetchArray($result_file))) {
            $data['files'][$i]= $file;
            $i++;
        }
        $teachers_arr[$id] = $data;
    }
    // var_dump( $teachers_arr);
    $xoopsTpl->assign('teachers', $teachers_arr);
    $xoopsTpl->assign('web_url', XOOPS_URL);
    $sql = 'select * from `' . $xoopsDB->prefix('kw_club_class') . "`
...
function pid_check($cardid)
{
    ...
    if ('1' != $gender && '2' != $gender && '8' != $gender && '9' != $gender && 'A' !== $gender && 'B' !== $gender && 'C' !== $gender && 'D' !== $gender) {
        return false;
    } //驗證性別
...
function update_class_enable($class_id, $class_enable)
{
    ...
    `class_isopen` = '{$class_enable}' where `class_id` = '$class_id'";
