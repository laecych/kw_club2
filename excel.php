...
$objActSheet->setCellValue("J{$i}", _MD_KWCLUB_NUMBER);
$objActSheet->setCellValue("K{$i}", _MD_KWCLUB_REG_PARENT);
$objActSheet->setCellValue("L{$i}", _MD_KWCLUB_REG_TEL);
$objActSheet->setCellValue("M{$i}", _MD_KWCLUB_REG_DATETIME);
...
$objActSheet->getColumnDimension('J')->setWidth(6);
...
$sql = 'select a.`reg_sn`,b.`club_year`,b.`class_title`,b.`class_money`,b.`class_fee`,a.`reg_uid`,a.`reg_name`,a.`reg_grade`,a.`reg_class`,a.`reg_number`,a.`reg_parent`,a.`reg_tel`,a.`reg_datetime` from `' . $xoopsDB->prefix('kw_club_reg') . '` as a
...
$objActSheet->getStyle("A1:M{$n}")->getBorders()->getAllborders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN)->getColor()->setRGB('000000');
