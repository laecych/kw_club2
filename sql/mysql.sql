...
CREATE TABLE `kw_club_info` (
  ...
  `club_isshow` enum('1','0') NOT NULL DEFAULT '0' COMMENT '報名顯示',
  ...
  `club_sort` smallint(6) unsigned NOT NULL DEFAULT '1' COMMENT '顯示順序',
  PRIMARY KEY (`club_id`),
...
CREATE TABLE `kw_club_class` (
  ...
  `class_ischecked` enum('1','0','') NOT NULL DEFAULT '' COMMENT '是否開班',
  ...
  `class_sort` smallint(6) unsigned NOT NULL DEFAULT '1' COMMENT '課程排序',
  PRIMARY KEY (`class_id`),
  UNIQUE KEY `club_year_class_num` (`club_year`,`class_num`),
  FOREIGN KEY (`club_year`) REFERENCES `xx_kw_club_info` (`club_year`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `kw_club_reg` (
  ...
  `reg_number` tinyint(3) NOT NULL COMMENT '報名者座號',
  ...
  FOREIGN KEY (`class_id`) REFERENCES `xx_kw_club_class` (`class_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `kw_club_files_center` (
  ...
  FOREIGN KEY (`col_sn`) REFERENCES `xx_kw_club_teacher` (`teacher_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
