    ...
    <div class="row">
        ...
        <div class="col-sm-2" style="padding-top: 40px;">
            <{if $smarty.session.isclubAdmin || $smarty.session.isclubUser}>
                <a href="<{$xoops_url}>/modules/kw_club/club.php?club_year=<{$block.club_year}>" class="btn btn-primary btn-block" <{if !$block.can_operate}>data-toggle="tooltip" data-placement="bottom" data-bs-toggle="tooltip" data-bs-placement="bottom" title="<{$smarty.const._MB_KWCLUB_OVER_END_TIME}>" disabled<{/if}>><i class="fa fa-plus" aria-hidden="true"></i>
            ...
            <{foreach from=$block.all_class_content item=data}>
                <ul id="tr_<{$block.data.class_id}>">
                    ...
                    <li class="vm w1 text-center">
                        <span data-toggle="tooltip" data-placement="bottom" data-bs-toggle="tooltip" data-bs-placement="bottom" <{if $data.class_fee}>style="color: #ad168a;"  title="<{$smarty.const._MB_KWCLUB_CLASS_MONEY}> <{$data.class_money}> <{$smarty.const._MB_KWCLUB_DOLLAR}> + <{$smarty.const._MB_KWCLUB_CLASS_FEE}> <{$data.class_fee}> <{$smarty.const._MB_KWCLUB_DOLLAR}>"<{/if}>><{$data.class_money}><{if $data.class_fee}> (<{$data.class_fee}>) <{/if}><{$smarty.const._MB_KWCLUB_DOLLAR}></span>
                    </li>

                    <!--已報名人數-->
                    <li class="vm w1 text-center">
                        <{if $data.class_regnum >= $data.class_member}>
                            <span class="circle" data-toggle="tooltip" data-placement="bottom" data-bs-toggle="tooltip" data-bs-placement="bottom" title="<{$smarty.const._MB_KWCLUB_NUMBER_OF_APPLICANTS}> <{$data.class_regnum}> <{$smarty.const._MB_KWCLUB_PEOPLE}>"><{$smarty.const._MB_KWCLUB_FULL}></span>
                        <{else}>
                            <{$data.class_regnum}> /
                            <{$data.class_member}>
                        <{/if}>
                    ...
                    <li class="vm w1 text-center">
                        <{if $smarty.session.isclubAdmin}>
                            <{if $data.class_ischecked!=1}>
                                <a href="<{$xoops_url}>/modules/kw_club/club.php?op=class_enable&class_id=<{$data.class_id}>" class="btn btn-sm btn-success" data-toggle="tooltip" data-placement="bottom" data-bs-toggle="tooltip" data-bs-placement="bottom" title="<{$smarty.const._MB_KWCLUB_CLASS_ENABLE_DESC}>"><{$smarty.const._MB_KWCLUB_CLASS_ENABLE}></a>
                            <{else}>
                                <a href="<{$xoops_url}>/modules/kw_club/club.php?op=class_unable&class_id=<{$data.class_id}>" class="btn btn-sm btn-danger" data-toggle="tooltip" data-placement="bottom" data-bs-toggle="tooltip" data-bs-placement="bottom" title="<{$smarty.const._MB_KWCLUB_CLASS_UNABLE_DESC}>"><{$smarty.const._MB_KWCLUB_CLASS_UNABLE}></a>
                            <{/if}>
                            <a href="<{$xoops_url}>/modules/kw_club/club.php?op=class_blank&class_id=<{$data.class_id}>" class="btn btn-sm btn-secondary" data-toggle="tooltip" data-placement="bottom" data-bs-toggle="tooltip" data-bs-placement="bottom" title="<{$smarty.const._MB_KWCLUB_CLASS_BLANK_DESC}>"><{$smarty.const._MB_KWCLUB_CLASS_BLANK}></a>
                        <{/if}>

                        <{if $smarty.session.isclubAdmin || $block.uid == $data.class_uid}>
                            <a href="<{$xoops_url}>/modules/kw_club/club.php?class_id=<{$data.class_id}>" class="btn btn-sm btn-warning" <{if !$block.can_operate}>data-toggle="tooltip" data-placement="bottom" data-bs-toggle="tooltip" data-bs-placement="bottom" title="<{$smarty.const._MB_KWCLUB_OVER_END_TIME}>" disabled<{/if}>><{$smarty.const._TAD_EDIT}></a>
                            <{if $data.class_regnum == 0}>
                                <a href="javascript:delete_class_func(<{$data.class_id}>);" class="btn btn-sm btn-danger" <{if !$block.can_operate}>data-toggle="tooltip" data-placement="bottom" data-bs-toggle="tooltip" data-bs-placement="bottom" title="<{$smarty.const._MB_KWCLUB_OVER_END_TIME}>" disabled<{/if}>><{$smarty.const._TAD_DEL}></a>
                            <{/if}>
                        <{else}>
                            <{if $data.is_full}>
                                <a href="#" class="btn btn-danger btn-sm disabled" data-toggle="tooltip" data-placement="bottom" data-bs-toggle="tooltip" data-bs-placement="bottom" title="<{$smarty.const._MB_KWCLUB_NUMBER_OF_APPLICANTS}> <{$data.class_regnum}> <{$smarty.const._MB_KWCLUB_PEOPLE}>"><i class="fa fa-user-plus" aria-hidden="true"></i>
